...
\begin{definicion}Pendulum \cite{Pendulum}: It’s a physical system that can oscillate under the gravitational action or other physical characteristic (elasticity, for example) and that is configured by a mass suspended from a point or a horizontal axis by a wire, a rod, or other device that is used to measure time. \newline
\end{definicion}
...
\begin{itemize}
	...
	\item Header pin modules for the MPU6050 connection previously described. (Figure \ref{fig:mpu6050_connector}).
		...
		\item A great part of actual microcontrollers works at 3.3-5V but the input voltage supported can go high to 12V, because of that it’s taken advantage of the LIPO battery power and a new connector with two header pin is implemented that goes to VCC and GND which will power the microcontroller.	
...
\begin{itemize}
	\item data[7:0]:Consists in a buffer in which bits are being stored when it’s necessary until we have the byte. It’s important that the following module knows when the byte is prepared for its capture.
...
Applying this signal, for example, to a classic DC motor the amount of energy that is applied to the load is varied, the motor in this case. It simply works as a switch in which a high logical level is open and a low level closed. If it’s managed to vary the time the motor is being charged and the time in where there’s no current, you can control its speed. \newline
...
Se muestran un conjunto de vídeos demostrativos en \cite{self2} del correcto funcionamiento del self-balancing robot, así como de su proceso hasta el resultado final en \cite{self1} \cite{self3}. \newline 
Para el desarrollo de la estructur mecánica, se muestra en \cite{print1} un vídeo representativo de su impresión.
